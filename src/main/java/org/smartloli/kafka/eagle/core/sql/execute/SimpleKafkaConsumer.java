...
public class SimpleKafkaConsumer {
    ...
    public static List<JSONArray> start(KafkaSqlDomain kafkaSql) {
        LOG.info("invoke start::kafkaSql={}", gson.toJson(kafkaSql));
    ...
    private JSONArray run(long a_maxReads, String a_topic, int a_partition, List<HostsDomain> a_seedBrokers)
            throws Exception {
        LOG.info("invoke run::a_maxReads={}, a_topic={}, a_partition={}, a_seedBrokers={}", a_maxReads, a_topic,
                a_partition, a_seedBrokers);
        ...
        while (a_maxReads > 0) {
            ...
            LOG.info("goint to consumer.fetch(req) , topic:{},partition:{},readOffset:{},fetchSize:{}", a_topic,
                    a_partition, readOffset, fetch_size);
            FetchRequest req = new FetchRequestBuilder().clientId(clientName)
                    .addFetch(a_topic, a_partition, readOffset, fetch_size).build();
            ...
            if (fetchResponse.hasError()) {
                ...
                LOG.error(
                        "[SimpleKafkaConsumer.run()] - Error fetching data from the Broker:" + leadBroker + ":" + a_port
                                + " Reason: " + code);
                if (numErrors > 5) {
                    break;
                }
            ...
            ByteBufferMessageSet messageAndOffsets = fetchResponse.messageSet(a_topic, a_partition);
            ...
            for (MessageAndOffset messageAndOffset : messageAndOffsets) {
                ...
                if (messageAndOffset.message() != null && messageAndOffset.message().payload() != null) {
                ...
                topic.put("msg", content);
                topics.add(topic);

                numRead++;
                a_maxReads--;
            }
            LOG.info("num read:{},numerr:{}", numRead, numErrors);
            numErrors = 0;

            if (numRead == 0) {
                LOG.info("read num is 0 ,sleep");
                try {
                    Thread.sleep(1000);
        ...
        LOG.info("got messages :{}", gotMessages);
        if (consumer != null) {
            consumer.close();
        }
        return topics;
     ...
     * @return String
     *
     * @throws Exception find next leader broker
     */
    ...
    private PartitionMetadata findLeader(List<HostsDomain> a_seedBrokers, String a_topic, int a_partition) {
        ...
        loop:
        for (HostsDomain seed : a_seedBrokers) {
