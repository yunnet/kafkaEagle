...
public class ZkServiceImpl implements ZkService {
                ...
                for (String topic : topics) {
                    try {
                        ...
                        Tuple2<Option<String>, Stat> tuple =
                                ZkUtils.apply(zkc, false).readDataMaybeNull(path + "/" + group + "/" + topic);
    ...
    public void insert(String clusterAlias, List<OffsetsLiteDomain> list) {
        ...
        for (OffsetsLiteDomain offset : list) {
            ...
            object.put("lagsize", offset.getLogSize()); // 事实上是logsize
            ...
            update(clusterAlias, target.toJSONString(),
                    STORE_OFFSETS + "/" + offset.getGroup() + "/" + offset.getTopic());
        }
    }
     ...
     *
     * @param alarm New configure object.
     *
     * @return Integer.
     */
     ...
     *
     * @param data Update datasets.
     * @param path Update datasets path.
     */
    private void update(String clusterAlias, String data, String path) {
        ...
        if (!ZkUtils.apply(zkc, false).pathExists(KE_ROOT_PATH + "/" + path)) {
            ZkUtils.apply(zkc, false)
                    .createPersistentPath(KE_ROOT_PATH + "/" + path, "", ZkUtils.apply(zkc, false).DefaultAcls());
        }
        if (ZkUtils.apply(zkc, false).pathExists(KE_ROOT_PATH + "/" + path)) {
            ZkUtils.apply(zkc, false)
                    .updatePersistentPath(KE_ROOT_PATH + "/" + path, data, ZkUtils.apply(zkc, false).DefaultAcls());
        }
        if (zkc != null) {
            zkPool.release(clusterAlias, zkc);
            zkc = null;
