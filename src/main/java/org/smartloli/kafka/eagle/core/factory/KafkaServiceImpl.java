...
public class KafkaServiceImpl implements KafkaService {
    ...
    public List<String> findTopicPartition(String clusterAlias, String topic) {
        ...
        Seq<String> brokerTopicsPaths =
                ZkUtils.apply(zkc, false).getChildren(BROKER_TOPICS_PATH + "/" + topic + "/partitions");
    ...
    public Map<String, List<String>> getActiveTopic(String clusterAlias) {
        ...
        try {
            ...
            for (String group : groups) {
                scala.collection.mutable.Map<String, scala.collection.immutable.List<ConsumerThreadId>> topics =
                        ZkUtils.apply(zkc, false).getConsumersPerTopic(group, false);
    ...
    public String getAllBrokersInfo(String clusterAlias) {
        ...
        if (ZkUtils.apply(zkc, false).pathExists(BROKER_IDS_PATH)) {
            ...
            for (String ids : brokerIdss) {
                try {
                    Tuple2<Option<String>, Stat> tuple =
                            ZkUtils.apply(zkc, false).readDataMaybeNull(BROKER_IDS_PATH + "/" + ids);
    ...
    public String getAllPartitions(String clusterAlias) {
        ...
        if (ZkUtils.apply(zkc, false).pathExists(BROKER_TOPICS_PATH)) {
            ...
            for (String topic : topics) {
                try {
                    Tuple2<Option<String>, Stat> tuple =
                            ZkUtils.apply(zkc, false).readDataMaybeNull(BROKER_TOPICS_PATH + "/" + topic);
    ...
    public Map<String, List<String>> getConsumers(String clusterAlias, PageParamDomain page) {
        ...
        try {
            if (page.getSearch().length() > 0) {
                ...
                int offset = 0;
                for (String group : groups) {
                    if (offset < (page.getiDisplayLength() + page.getiDisplayStart()) && offset >= page
                            .getiDisplayStart()) {
                        String path = CONSUMERS_PATH + "/" + group + "/owners";
                        if (ZkUtils.apply(zkc, false).pathExists(path)) {
                            Seq<String> owners = ZkUtils.apply(zkc, false).getChildren(path);
     ...
     *
     * @param hosts     Zookeeper host list.
     * @param topic     Appoint topic.
     * @param partition Appoint partition.
     *
     * @return Long.
     */
    public long getLogSize(List<String> hosts, String topic, int partition) {
        ...
        try {
            ...
            kafka.javaapi.OffsetRequest request =
                    new kafka.javaapi.OffsetRequest(requestInfo, OffsetRequest.CurrentVersion(), clientName);
    ...
    public Map<String, Object> create(String clusterAlias, String topicName, String partitions, String replic) {
        ...
        if ("kafka".equals(formatter)) {
            ...
            AdminUtils.createTopic(zkUtils, topicName, Integer.parseInt(partitions), Integer.parseInt(replic),
            ...
            String[] options =
                    new String[] {"--create", "--zookeeper", zks, "--partitions", partitions, "--topic", topicName,
                            "--replication-factor", replic};
            TopicCommand.main(options);
        }
        targets.put("status", "success");
        targets.put("info", "Create topic[" + topicName + "] has successed,partitions numbers is [" + partitions
        ...
        if (!sql.startsWith("select")) {
            ...
            if (matcher.find()) {
                ...
                if (matcher.group(2).trim().startsWith("\"partition\"")) {
                    String[] columns =
                            matcher.group(2).trim().split("in")[1].replace("(", "").replace(")", "").trim().split(",");
    ...
    public String getKafkaConsumer(String clusterAlias) {
        LOG.info("invoke getKafkaConsumer::clusterAlias={}", clusterAlias);
        ...
        try {
            ...
            scala.collection.immutable.Map<Node, scala.collection.immutable.List<GroupOverview>> opts =
                    adminClient.listAllConsumerGroups();
    ...
    private JSONArray getKafkaMetadata(String bootstrapServers, String group) {
        LOG.info("invoke getKafkaMetadata::bootstrapServers={}, group={}", bootstrapServers, group);
        Properties prop = new Properties();
        ...
        try {
            ...
            LOG.error("Get kafka consumer metadata has error, msg is ,", e);
        }
    ...
    public String getKafkaOffset(String clusterAlias) {
        LOG.info("invoke getKafkaOffset::clusterAlias={}", clusterAlias);
        if (KafkaOffsetGetter.multiKafkaConsumerOffsets.containsKey(clusterAlias)) {
            JSONArray targets = new JSONArray();
            for (Entry<GroupTopicPartition, OffsetAndMetadata> entry : KafkaOffsetGetter.multiKafkaConsumerOffsets
                    .get(clusterAlias).entrySet()) {
                JSONObject object = new JSONObject();
    ...
    public long getKafkaLogSize(String clusterAlias, String topic, int partitionid) {
        ...
        Map<TopicPartition, Long> logsize = consumer.endOffsets(Collections.singleton(tp));
        consumer.close();
